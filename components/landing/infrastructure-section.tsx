...
const trustLevels = [
  { name: "Verified Pro", score: "95-100", status: "elite" },
  { name: "Trusted", score: "80-94", status: "verified" },
  { name: "Rising", score: "50-79", status: "building" },
  { name: "New User", score: "0-49", status: "starting" },
];
...
export function InfrastructureSection() {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setActiveRegion((prev) => (prev + 1) % trustLevels.length);
    }, 3000);
          ...
          <div className="grid lg:grid-cols-[auto_1fr] gap-8 lg:gap-16 items-stretch">
            ...
            <div className="flex flex-col justify-center">
              ...
              <p className={`mt-8 text-xl text-muted-foreground leading-relaxed max-w-lg transition-all duration-1000 delay-100 ${
                ...
                Every transaction builds your reputation score on the blockchain.
                Visible trust level for all users, forever.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className={`lg:col-span-2 relative p-8 lg:p-12 border border-foreground/10 bg-foreground/[0.02] overflow-hidden transition-all duration-700 ${
            ...
            <div className="relative z-10">
              <div className="flex items-baseline gap-2 mb-4">
                ...
                <span className="text-2xl text-muted-foreground">trust score</span>
              </div>
              <p className="text-muted-foreground max-w-md">
                Your reputation score is permanently stored on Solana blockchain, visible to all users.
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-6">
            <div className={`p-8 border border-foreground/10 bg-foreground/[0.02] transition-all duration-700 delay-100 ${
              ...
              <span className="text-5xl lg:text-6xl font-display">5000+</span>
              <span className="block text-sm text-muted-foreground mt-2">Verified Users</span>
            </div>
            
            <div className={`p-8 border border-foreground/10 bg-foreground/[0.02] transition-all duration-700 delay-200 ${
              isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
            }`}>
              <span className="text-5xl lg:text-6xl font-display">$2.5M+</span>
              <span className="block text-sm text-muted-foreground mt-2">Total Transacted</span>
            </div>
          </div>
        </div>
            ...
            >
              <div className="flex items-center gap-2 mb-3">
                <span className={`w-2 h-2 rounded-full transition-colors ${
                  activeRegion === index ? "bg-[#4ade80]" : "bg-foreground/20"
                }`} />
                <span className="text-xs font-mono text-muted-foreground uppercase tracking-wider">
                  {level.status}
                </span>
              </div>
              <span className="font-medium block mb-1">{level.name}</span>
              <span className="text-sm text-muted-foreground">Score: {level.score}</span>
            </div>
          ))}
        </div>
      </div>
